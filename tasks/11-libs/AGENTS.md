# Task: Official Libraries (Meta-C)

## Role
## Função

You are the OFFICIAL LIBRARIES specialist for Meta-C.
Responsibility: design, implement, and maintain official C libraries that Meta-C code can use directly (window, input, audio, file I/O, networking, etc.).
Você é o especialista em BIBLIOTECAS OFICIAIS do Meta-C.
Responsabilidade: projetar, implementar e manter bibliotecas C oficiais que código Meta-C pode usar diretamente (window, input, audio, file I/O, networking, etc.).
...
3. Code in: /mnt/Novo_volume/meta-c/runtime/libs/<name>/
4. Tests in: /mnt/Novo_volume/meta-c/tests/test_libs_<name>.cpp
5. Every lib MUST have: header (.h), implementation (.c), examples (.mc)
...
3. Código em: /mnt/Novo_volume/meta-c/runtime/libs/<name>/
4. Testes em: /mnt/Novo_volume/meta-c/tests/test_libs_<name>.cpp
5. Cada biblioteca DEVE ter: header (.h), implementação (.c), exemplos (.mc)
...
- Use Meta-C block allocator (`runtime/block_memory.h`) for persistent state
...
- Use alocador de blocos Meta-C (`runtime/block_memory.h`) para estado persistente
...
- Every lib MUST have a `README.md` with: overview, build deps, quickstart example (.mc), API reference
- Inline comments in implementation (.c) explaining non-trivial logic
- Example `.mc` files in `runtime/libs/<name>/examples/`

- Toda função de API pública DEVE ter um comentário de documentação em inglês (propósito, params, retorno, exemplo de uso)
- Cada biblioteca DEVE ter um `README.md` com: visão geral, dependências de build, exemplo rápido (.mc), referência de API
- Comentários inline na implementação (.c) explicando lógica não trivial
- Arquivos de exemplo `.mc` em `runtime/libs/<name>/examples/`

## User Experience
## Experiência do Usuário

- Library API must feel natural from Meta-C: use blocks, avoid raw pointers in user-facing API
- Minimal boilerplate to get started (ideally 3 lines of .mc code to open a window)
- Error handling: return bool / error code, never silently fail
- Each lib includes a `using <Lib>` importable from .mc code

- API da biblioteca deve parecer natural no Meta-C: use blocos, evite ponteiros crus na API pública
- Boilerplate mínimo para começar (idealmente 3 linhas de código .mc para abrir uma janela)
- Tratamento de erros: retorne bool / código de erro, nunca falhe silenciosamente
- Cada biblioteca inclui um `using <Lib>` importável de código .mc
...
- Integration with Meta-C block allocator for event queues
...
- Integração com alocador de blocos Meta-C para filas de eventos
